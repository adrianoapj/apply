...
const Main = (props) => {
  ...
  const submitStatusProps = {
    unopened: { color: 'primary', children: intl.formatMessage({id: "READY_FOR_YOU"}) },
    incomplete: { color: 'warning', children: intl.formatMessage({id: "IN_PROGRESS_WITH_PERIOD"}) },
    complete: { color: 'info', children: intl.formatMessage({id: "READY_TO_SUBMIT"}) },
    submitted: { color: 'success', children: intl.formatMessage({id: "SUBMITTED_WITH_EXCLAMATION_MARK"}) }
  }[submitButtonStatus]
  ...
  return (
    <Container maxWidth={52} py={4}>
      ...
      <Sheet mt={4} p={[3, 4, 5]}>
        ...
        */}
        <Section
          href={`/${locale}/club?id=${id}`}
          name={
            <Box>
              <Status type={applicationStatus()} />
              <Text>{app.high_school_name || intl.formatMessage({id: "CLUB_APPLICATION"})}</Text>
            </Box>
          }
        />
        <Section
          href={`/${locale}/leader?id=${leaderProfile.id}`}
          name={
            <Box>
        ...
        {coLeaderProfiles.length === 0 && (
          <Text py={4} color="muted" align="center" fontSize={3}>
            <Text.span bold>{intl.formatMessage({id: "NO_COLEADERS_YET"})}</Text.span>
          </Text>
        )}
            ...
            <SectionIcon
              ...
              onClick={(e) => {
                if (
                  ...
                  confirm(
                    intl.formatMessage({id: "ARE_YOU_SURE_REMOVE_AS_A_TEAM_MEMBER"}, {team_member: profile.user.email})
                  )
                ) {
                  api
