...
const SaveStatusIcon = ({ saved }) => {
  ...
  return (
    <SaveBaseIcon
      ...
      title={saved ? intl.formatMessage({id: "SAVED"}) : intl.formatMessage({id: "SAVING"})}
      size={36}
      m={2}
      saved={saved}
    />
...
class ConfirmClose extends Component {
  componentDidMount() {
    ...
    window.onbeforeunload = () =>
      window.confirm(
        intl.formatMessage({id: "BEFORE_LEAVING_WARNING"})
      )
  }
    ...
    <Text.span
      ...
      children={
        actual === 0
          ...
          : intl.formatMessage({id: "AIM_FOR_BETWEEN_WITH_ACTUAL"}, {actual, min, max})
      }
    />
  )
}
